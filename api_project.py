...
try:
    if load_dotenv:
        ...
        print("WARNING  python-dotenv not available. Environment variables from .env will not be loaded. Install with: pip install python-dotenv")
...
try:
    ...
    hopsworks = None
    print(
        "WARNING  hopsworks import failed: {}\n".format(e)
...
def http_get_json(url: str) -> Dict:
    ...
    try:
        ...
        print(f"WARNING  Timeout error for URL: {url[:100]}...")
        raise RuntimeError("API request timed out. Try again later or reduce data range.")
    except requests.exceptions.ConnectionError:
        print(f"WARNING  Connection error for URL: {url[:100]}...")
...
def save_pollutant_features_to_hopsworks(df: pd.DataFrame, feature_group_name: str = "aqi_pollutants", version: int = 1) -> None:
    ...
    if df is None or df.empty:
        print("WARNING  No data to save to Hopsworks (empty dataframe).")
        return
    ...
    if hopsworks is None:
        print("WARNING  hopsworks Python package not installed. Install with: pip install hopsworks")
        return
    ...
    if not api_key:
        print("WARNING  Hopsworks API key not found in environment (HOPSWORKS_API_KEY or hopswork_api). Skipping upload.")
        return
    try:
        ...
        print(f"WARNING  Failed to save to Hopsworks Feature Store: {e}")
def main() -> None:
    """
    ...
    try:
        ...
        print(f"WARNING  Failed to fetch 1-year data: {e}")
        print("Retry Trying with 3-month data instead...")
